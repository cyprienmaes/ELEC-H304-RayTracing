...
double norme_coeff_reflexion(double theta_i, float eps_rel, float sigma, float epaisseur){
    ...
    double theta_t = theta_t_snell(theta_i, eps_0, eps_rel*eps_0); //calcul angle de transmission
    double s = epaisseur/cos(theta_t); // distance de propagation dans le mur (pour un "aller de rayon")
    ...
    double complex coef_perp = (z3*cos(theta_i) - z1*cos(theta_t)) / (z3*cos(theta_i) + z1*cos(theta_t)); //coefficient de reflexion pour
                                                                                                     //polarisation perpendiculaire

    double complex partie_exp = cexp(-2*gamma*s) * cexp(I*beta*2*s*sin(theta_t)*sin(theta_i));  //Partie exponentielle de la formule du
